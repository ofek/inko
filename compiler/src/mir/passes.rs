...
impl<'a> GenerateDropper<'a> {
    ...
    fn async_class(&mut self) -> MethodId {
        ...
        let loc = lower.new_location(self.location);
...
impl<'a> LowerToMir<'a> {
    ...
    fn define_module_method(
        ...
        let lower =
            LowerMethod::new(self.state, self.mir, self.module, &mut method);
        let loc = lower.new_location(node.location);
...
impl<'a> LowerMethod<'a> {
    ...
    fn lower_method_body(
        ...
        for (index, node) in nodes.into_iter().enumerate() {
            ...
            let loc = self.new_location(node.location());
    ...
    fn tuple_literal(&mut self, node: hir::TupleLiteral) -> RegisterId {
        ...
        for (index, val) in node.values.into_iter().enumerate() {
            ...
            let loc = self.new_location(val.location());
    ...
    fn string_literal(&mut self, mut node: hir::StringLiteral) -> RegisterId {
        match node.values.len() {
            0 => self
                .string_text(String::new(), self.new_location(node.location)),
            1 => match node.values.pop().unwrap() {
                hir::StringValue::Text(n) => {
                    self.string_text(n.value, self.new_location(n.location))
                }
    ...
    fn call(&mut self, node: hir::Call) -> RegisterId {
        ...
        let loc = self.new_location(node.name.location);
        let reg = match node.kind {
            ...
            types::CallKind::ClassInstance(info) => {
                ...
                {
                    let loc = self.new_location(arg.location());
                    let val =
    ...
    fn call_method(
        ...
        let ins_loc = self.new_location(location);
    ...
    fn noop(&mut self, location: Location) -> RegisterId {
        self.get_nil(self.new_location(location))
    }
    ...
    fn decision(
        ...
        match node {
            ...
            pmatch::Decision::Guard(guard_node, ok, fail) => {
                ...
                let guard_loc = self.new_location(guard_node.location());
    ...
    fn decision_body(
        ...
        let loc = self.new_location(body_loc);
    ...
    fn identifier(&mut self, node: hir::IdentifierRef) -> RegisterId {
        let loc = node.location;
        let ins_loc = self.new_location(loc);
    ...
    fn new_location(&self, location: Location) -> InstructionLocation {
        InstructionLocation::new(self.method.id, location)
    }
}
